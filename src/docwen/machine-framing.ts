...
export class MachineFrameDecoder {
  ...
  feed(chunk: Buffer): JsonObject[] {
    ...
    while (this.buffer.length > 0) {
      ...
      if (headerEnd < 0) {
        if (this.buffer.indexOf("\n\n") >= 0) throw new Error("docwen_machine_invalid_frame_header");
        if (this.buffer.length > HEADER_LIMIT_BYTES) throw new Error("docwen_machine_invalid_frame_header");
        break;
      }
      const headerBytes = this.buffer.subarray(0, headerEnd + 4);
      if (headerBytes.length > HEADER_LIMIT_BYTES) throw new Error("docwen_machine_invalid_frame_header");
      const match = HEADER_PATTERN.exec(headerBytes.toString("latin1"));
      ...
      const frameEnd = headerBytes.length + contentLength;
      if (this.buffer.length < frameEnd) break;
      const body = this.buffer.subarray(headerBytes.length, frameEnd);
