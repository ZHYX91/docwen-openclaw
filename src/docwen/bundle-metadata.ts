import { isJsonObject, type JsonObject } from "./machine-framing.js";
...
export function validateBundleFields(bundle: JsonObject, invalid: Invalid): void {
  fields(
    bundle,
    ["schema", "bundle_id", "task_id", "producer", "layout_schema", "artifacts", "entries", "relations"],
    [],
    "bundle",
    invalid,
  );
  ...
  const producer = fields(
    bundle.producer,
    ["name", "product_version", "machine_protocol"],
    [],
    "producer",
    invalid,
  );
  ...
  for (const raw of bundle.relations as unknown[]) {
    const relation = fields(
      raw,
      ["type", "source_artifact_id", "target_artifact_id", "role"],
      ["ordinal", "page_fragment", "page_resource"],
      "relation",
      invalid,
    );
    ...
    if (Object.hasOwn(relation, "page_fragment")) {
      fields(
        relation.page_fragment,
        ["fragment_kind", "page_index", "page_count", "ocr_status", "source_page"],
        [],
        "page_fragment",
        invalid,
      );
    }
    if (Object.hasOwn(relation, "page_resource"))
      fields(relation.page_resource, ["source_page"], [], "page_resource", invalid);
  }
}
...
export function validateBundlePages(bundle: JsonObject, invalid: Invalid): void {
  ...
  const fail = (code: string): never => {
    throw invalid(`Artifact Bundle physical pages: ${code}.`);
  };
  const byId = new Map(artifacts.map((item) => [item.artifact_id, item]));
  const primary = new Set(
    entries
      .filter((entry) => entry.role === "primary" && byId.get(String(entry.artifact_id))?.kind === "document")
  ...
  for (const relation of relations) {
    ...
    if (isPage) {
      ...
      if (
        data.fragment_kind !== "page" ||
        !positive(data.page_index) ||
        !positive(data.page_count) ||
        !positive(data.source_page) ||
        data.page_index > data.page_count ||
        data.source_page > data.page_count ||
        typeof data.ocr_status !== "string" ||
        !OCR_STATUSES.has(data.ocr_status)
      )
        fail("invalid_page_range");
    ...
    if (
      relation.page_resource !== undefined &&
      (relation.type !== "resource_of" || !["image", "original", "preview"].includes(String(relation.role)))
    )
      fail("unexpected_page_semantics");
  }
  ...
  for (const relation of relations) {
    if (relation.type !== "resource_of" || !["image", "original", "preview"].includes(String(relation.role)))
      continue;
    const resource = relation.page_resource;
    if (resource !== undefined && (!isJsonObject(resource) || !positive(resource.source_page)))
      fail("invalid_page_range");
    const targetPage = pageByArtifact.get(relation.target_artifact_id);
    if (targetPage) {
      if (!isJsonObject(resource) || resource.source_page !== targetPage.source_page)
