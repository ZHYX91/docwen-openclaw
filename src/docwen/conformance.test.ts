...
describe("pinned DocWen normative contract snapshot", () => {
    ...
    async (section) => {
      ...
      await expect(
        validateArtifactBundle(
          bundle,
          path.join(contractRoot, "unavailable-staging"),
          bundle.task_id as string,
        ),
  ...
  it.each(
    fixtures.filter(
      (fixture) => fixture.document_type === "bundle" && /page|resource_page/u.test(fixture.error_code ?? ""),
    ),
  )("$path: rejects physical-page errors before file access", async (fixture) => {
  ...
  it("accepts an explicit audit manifest owned by the preferred resource", async () => {
    ...
    bundle.artifacts = [
      ...
      {
        ...resource,
        artifact_id: "resource.audit",
        locator: "docwen-node.json",
        logical_path: "docwen-node.json",
    ...
    bundle.relations = [
      {
        type: "resource_of",
        source_artifact_id: "resource.audit",
        target_artifact_id: resource.artifact_id,
        role: "manifest",
      },
    ];
    await writeFile(path.join(root, "image.png"), bytes);
    await writeFile(path.join(root, "docwen-node.json"), bytes);
    expect((await validateArtifactBundle(bundle, root, bundle.task_id as string)).artifacts).toHaveLength(2);
    (bundle.artifacts as JsonObject[])[1]!.suggested_name = "unrelated.json";
    await expect(validateArtifactBundle(bundle, root, bundle.task_id as string)).rejects.toMatchObject({
      code: expect.stringMatching(/integrity_error$/u),
    });
  });
  it("has the recorded complete inventory and digests", () => {
    ...
    const inventory = readdirSync(contractRoot, { recursive: true, withFileTypes: true })
      ...
      .map((entry) =>
        path.relative(contractRoot, path.join(entry.parentPath, entry.name)).replaceAll("\\", "/"),
      );
  ...
  it.each(fixtures.filter((fixture) => fixture.document_type !== "framing"))(
    "$path: schema expectation",
    (fixture) => {
      const schema = manifest.schemas.find((item) => item.name === fixture.schema)!;
      const validator = ajv.getSchema(schema.id)!;
      const documents: unknown[] =
        fixture.document_type === "trace" ? (fixture.data as unknown[]) : [fixture.data];
    ...
    async (fixture) => {
      ...
      for (const artifact of bundle.artifacts as JsonObject[]) {
        ...
        if (
          locator.includes("\\") ||
          locator.includes(":") ||
          locator.split("/").some((segment) => ["", ".", ".."].includes(segment))
        )
          continue;
      ...
      const result = validateArtifactBundle(bundle, root, bundle.task_id as string);
      if (fixture.expect === "valid") {
        ...
        await expect(result).rejects.toMatchObject({
          code: expect.stringMatching(/(?:integrity|protocol)_error$/u),
        });
      }
    },
  );
